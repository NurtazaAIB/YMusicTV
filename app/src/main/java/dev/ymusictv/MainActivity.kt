...
@Composable private fun Home(api:YandexMusicApi, player:TvPlayer, wave:WaveSession, logout:()->Unit){
    var screen by remember{mutableStateOf(Screen.MUSIC)}; var name by remember{mutableStateOf("…")}; var now by remember{mutableStateOf<Track?>(null)}; val scope=rememberCoroutineScope()
    LaunchedEffect(Unit){runCatching{api.accountName()}.onSuccess{name=it}}
    var streamRetryFor by remember { mutableStateOf<String?>(null) }
    fun play(t:Track){scope.launch{runCatching{api.audioUrl(t.id)}.onSuccess{url->player.play(t,url);now=t;screen=Screen.PLAYER}.onFailure{}}}
    DisposableEffect(Unit){
        player.onEnded { scope.launch { runCatching { wave.next(player.exo.duration.coerceAtLeast(0)/1000,false) }.onSuccess { it?.let { t -> streamRetryFor=null; play(t) } } } }
        player.onError {
            val t = now
            if (t != null && streamRetryFor != t.id) {
                streamRetryFor = t.id
                scope.launch { runCatching { api.audioUrl(t.id) }.onSuccess { freshUrl -> player.play(t, freshUrl) } }
            }
        }
        onDispose { }
    }
    Column(Modifier.onPreviewKeyEvent{e->if(e.nativeKeyEvent.action==KeyEvent.ACTION_DOWN){when(e.nativeKeyEvent.keyCode){KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE->{if(player.exo.isPlaying)player.exo.pause()else player.exo.play();true};KeyEvent.KEYCODE_MEDIA_PLAY->{player.exo.play();true};KeyEvent.KEYCODE_MEDIA_PAUSE->{player.exo.pause();true};else->false}}else false},verticalArrangement=Arrangement.spacedBy(15.dp)){
        Row(horizontalArrangement=Arrangement.spacedBy(10.dp)){Text("YMusic TV",fontSize=30.sp,color=Color.White,modifier=Modifier.weight(1f)); Text(name,fontSize=16.sp,color=Color.LightGray);now?.let{Button(onClick={screen=Screen.PLAYER}){Text("♫ ${it.title.take(24)}")}};Button(onClick=logout){Text("Выйти")}}
        ...
        when(screen){Screen.MUSIC->MusicScreen(api,wave){t->play(t)};Screen.AUDIO->AudioScreen(api,::play);Screen.SEARCH->SearchScreen(api,::play);Screen.PLAYER->PlayerScreen(api,player,wave,now,{t->play(t)}){screen=Screen.MUSIC}}
    }
}

@Composable private fun MusicScreen(api:YandexMusicApi,wave:WaveSession,play:(Track)->Unit){
    var tracks by remember{mutableStateOf<List<Track>>(emptyList())};var title by remember{mutableStateOf("Главная")};var error by remember{mutableStateOf<String?>(null)};var settings by remember{mutableStateOf(WaveSettings())};var sections by remember{mutableStateOf<List<HomeSection>>(emptyList())};val scope=rememberCoroutineScope()
    LaunchedEffect(Unit){ sections=runCatching{api.homeSections()}.getOrDefault(emptyList()) }
    fun waveStart(){scope.launch{runCatching{wave.start(settings)}.onSuccess{t->title="Моя Волна";if(t!=null){tracks=listOf(t);play(t)}}.onFailure{error=it.message}}}
    fun chart(){scope.launch{runCatching{api.chart()}.onSuccess{title="Чарт";tracks=it}.onFailure{error=it.message}}}
    Column(verticalArrangement=Arrangement.spacedBy(10.dp)){Text(title,fontSize=30.sp,color=Color.White)
        Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){Button(onClick={waveStart()}){Text("▶ Моя Волна")};Button(onClick={chart()}){Text("Чарт")};Button(onClick={settings=settings.copy(mood=if(settings.mood=="calm")"active" else "calm");waveStart()}){Text("Настроение: ${if(settings.mood=="calm")"спокойное" else "активное"}")};Button(onClick={settings=settings.copy(diversity=if(settings.diversity=="discover")"favorite" else "discover");waveStart()}){Text(if(settings.diversity=="discover")"Знакомое" else "Новое")}}
        error?.let{Text(it,color=Color(0xFFFF8A80))}
        if(tracks.isNotEmpty()) TrackList(tracks,play) else LazyColumn(verticalArrangement=Arrangement.spacedBy(12.dp),modifier=Modifier.fillMaxWidth().heightIn(max=455.dp)){
...
@Composable private fun PlayerScreen(api:YandexMusicApi,player:TvPlayer,wave:WaveSession,track:Track?,play:(Track)->Unit,back:()->Unit){
    var lyrics by remember{mutableStateOf<Lyrics?>(null)};var pos by remember{mutableLongStateOf(0L)};var message by remember{mutableStateOf<String?>(null)};val scope=rememberCoroutineScope()
    LaunchedEffect(track?.id){track?.let{lyrics=runCatching{api.lyrics(it.id)}.getOrNull()}}
    LaunchedEffect(Unit){while(true){pos=player.exo.currentPosition.coerceAtLeast(0);delay(250)}}
    val lines=lyrics?.lines.orEmpty();val idx=lines.indexOfLast{it.timeMs<=pos}.coerceAtLeast(0)
    Column(verticalArrangement=Arrangement.spacedBy(16.dp)){Button(onClick=back){Text("← Назад")};Text(track?.title?:"Плеер",fontSize=36.sp,color=Color.White);Text(track?.artist.orEmpty(),fontSize=22.sp,color=Color.LightGray)
        Row(horizontalArrangement=Arrangement.spacedBy(10.dp)){Button(onClick={player.exo.seekTo((player.exo.currentPosition-15000).coerceAtLeast(0))}){Text("−15 сек")};Button(onClick={if(player.exo.isPlaying)player.exo.pause()else player.exo.play()}){Text(if(player.exo.isPlaying)"Пауза" else "▶ Играть")};Button(onClick={scope.launch{runCatching{wave.likeCurrent()}.onSuccess{message="Добавлено в любимое ♥"}.onFailure{message=it.message}}}){Text("♥ Нравится")};Button(onClick={scope.launch{runCatching{wave.next(player.exo.currentPosition/1000,true)}.onSuccess{it?.let(play)}.onFailure{message=it.message}}}){Text("Следующий »")};Button(onClick={scope.launch{runCatching{wave.dislikeAndSkip(player.exo.currentPosition/1000)}.onSuccess{it?.let(play)}.onFailure{message=it.message}}}){Text("Не рекомендовать")}}
        message?.let{Text(it,color=Color(0xFFFFDB4D))}
        if(lines.isEmpty()) Text("Для этого трека синхронизированный текст не найден",color=Color.Gray) else Column(verticalArrangement=Arrangement.spacedBy(10.dp)){lines.getOrNull(idx-1)?.let{Text(it.text,fontSize=22.sp,color=Color.Gray)};lines.getOrNull(idx)?.let{Text(it.text,fontSize=32.sp,color=Color(0xFFFFDB4D))};lines.getOrNull(idx+1)?.let{Text(it.text,fontSize=22.sp,color=Color.LightGray)}}
    }
}
...
@Composable private fun AudioScreen(api:YandexMusicApi,play:(Track)->Unit){var tracks by remember{mutableStateOf<List<Track>>(emptyList())};var title by remember{mutableStateOf("Аудио")};val scope=rememberCoroutineScope();fun s(t:String,q:String){title=t;scope.launch{tracks=runCatching{api.searchTracks(q)}.getOrDefault(emptyList())}};Column(verticalArrangement=Arrangement.spacedBy(10.dp)){Text(title,fontSize=30.sp,color=Color.White);Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){Button(onClick={s("Қазақша ертегілер","қазақша ертегілер")}){Text("Қазақша ертегілер")};Button(onClick={s("Сказки на русском","детские сказки")}){Text("Сказки на русском")};Button(onClick={s("Аудиокниги","аудиокнига")}){Text("Аудиокниги")};Button(onClick={s("Подкасты","подкаст")}){Text("Подкасты")}};TrackList(tracks,play)}}
