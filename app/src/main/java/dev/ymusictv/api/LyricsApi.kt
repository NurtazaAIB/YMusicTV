...
class LyricsApi(
    ...
    companion object {
        ...
        private const val USER_AGENT = "MusicTV/0.9.7.1 AndroidTV"
    }

    suspend fun load(trackId: String): Lyrics? = withContext(Dispatchers.IO) {
        val id = trackId.substringBefore(':')
        // Prefer synchronized lyrics. A 403 from the signed endpoint is not fatal:
        // supplement still exposes ordinary full lyrics for many tracks.
        runCatching { loadSigned(id, "LRC") }.getOrNull()
            ?: loadSupplement(id)
            ?: runCatching { loadSigned(id, "TEXT") }.getOrNull()
    }

    private fun builder(url: String) = Request.Builder().url(url).header("User-Agent", USER_AGENT)
        .apply { tokenProvider()?.let { header("Authorization", "OAuth $it") } }
    private fun builder(url: okhttp3.HttpUrl) = Request.Builder().url(url).header("User-Agent", USER_AGENT)
        .apply { tokenProvider()?.let { header("Authorization", "OAuth $it") } }
    ...
    private fun loadSupplement(trackId: String): Lyrics? {
        ...
        val raw = lyric.optString("fullLyrics").ifBlank { lyric.optString("lyrics") }
    ...
    private fun loadSigned(trackId: String, format: String): Lyrics? {
        ...
        val url = "$API/tracks/$trackId/lyrics".toHttpUrl().newBuilder()
            .addQueryParameter("format", format).addQueryParameter("timeStamp", timestamp.toString())
        ...
        raw.lineSequence().forEach { source ->
            val matches=stamp.findAll(source).toList(); if(matches.isEmpty()) return@forEach
            val text=source.substring(matches.last().range.last+1).trim(); if(text.isBlank()) return@forEach
            matches.forEach { m ->
                val min=m.groupValues[1].toLongOrNull()?:0; val sec=m.groupValues[2].toLongOrNull()?:0; val f=m.groupValues[3]
                val ms=when(f.length){1->(f.toLongOrNull()?:0)*100;2->(f.toLongOrNull()?:0)*10;3->f.toLongOrNull()?:0;else->0}
    ...
    private fun plainLyrics(raw:String):Lyrics? {
        ...
        val lines=raw.lineSequence().map{it.trim()}.filter{it.isNotBlank()}.mapIndexed{i,s->LyricLine(i*5000L,s)}.toList()
