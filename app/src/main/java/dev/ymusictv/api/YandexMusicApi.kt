...
class YandexMusicApi(private val http: OkHttpClient = OkHttpClient()) {
    ...
    suspend fun requestDeviceCode(): DeviceCode = withContext(Dispatchers.IO) {
        ...
        executeJson(Request.Builder().url("$OAUTH/device/code").post(body).build()).let { j ->
            DeviceCode(j.getString("device_code"), j.getString("user_code"), j.getString("verification_url"), j.getInt("expires_in"), j.optInt("interval", 5))
        }
    }
    ...
    suspend fun pollDeviceToken(deviceCode: String): OAuthToken? = withContext(Dispatchers.IO) {
        ...
        http.newCall(req).execute().use { r ->
            ...
            if (!r.isSuccessful) when (j.optString("error")) {
    ...
    suspend fun accountName(): String {
        val result = getResult("$API/account/status")
        accountUid = result.optJSONObject("account")?.opt("uid")?.toString()
        return result.optJSONObject("account")?.optString("displayName")?.ifBlank { null }
            ?: result.optJSONObject("account")?.optString("login")?.ifBlank { null }
            ?: "Яндекс"
    }

    suspend fun myWave(queue: String? = null): WaveBatch {
        val suffix = buildString {
            append("?settings2=true")
            queue?.let { append("&queue=").append(URLEncoder.encode(it, "UTF-8")) }
        }
        val result = getResult("$API/rotor/station/user:onyourwave/tracks$suffix")
        val tracks = mutableListOf<Track>()
        val sequence = result.optJSONArray("sequence") ?: JSONArray()
        for (i in 0 until sequence.length()) {
            sequence.optJSONObject(i)?.optJSONObject("track")?.let { tracks += parseTrack(it) }
        }
        return WaveBatch(result.optString("batchId"), tracks)
    }

    suspend fun chart(option: String = ""): List<Track> {
        val suffix = if (option.isBlank()) "" else "/${URLEncoder.encode(option, "UTF-8")}"
        val result = getResult("$API/landing3/chart$suffix")
        val arr = result.optJSONArray("chart") ?: result.optJSONObject("chart")?.optJSONArray("tracks") ?: JSONArray()
        val out = mutableListOf<Track>()
        for (i in 0 until arr.length()) {
            val item = arr.optJSONObject(i) ?: continue
            val track = item.optJSONObject("track") ?: item
            if (track.has("id")) out += parseTrack(track)
        }
        return out
    }
    ...
    suspend fun setWaveSettings(settings: WaveSettings) = withContext(Dispatchers.IO) {
        val body = FormBody.Builder()
            .add("moodEnergy", settings.mood)
            .add("diversity", settings.diversity)
        ...
        http.newCall(authBuilder("$API/rotor/station/user:onyourwave/settings3").post(body).build()).execute().use { r ->
            ...
            if (text.isNotBlank() && text != "ok") {
                val result = runCatching { JSONObject(text).optString("result") }.getOrDefault("ok")
                if (result != "ok") throw IOException("Wave settings: ${text.take(160)}")
            }
        }
    }
    ...
    suspend fun lyrics(trackId: String): Lyrics? = withContext(Dispatchers.IO) {
        ...
        mac.init(SecretKeySpec("p93jhgh689SBReK6ghtw62".toByteArray(), "HmacSHA256"))
        val sign = Base64.encodeToString(mac.doFinal("$numericId$timestamp".toByteArray()), Base64.NO_WRAP)
        ...
        val download = result.optString("downloadUrl").ifBlank { return@withContext null }
        val text = executeText(Request.Builder().url(download).get().build())
        val re = Regex("\\[(\\d{1,2}):(\\d{2})(?:\\.(\\d{1,3}))?]\\s*(.*)")
        val lines = text.lineSequence().mapNotNull { line ->
            val m = re.matchEntire(line.trim()) ?: return@mapNotNull null
            val min = m.groupValues[1].toLong()
            val sec = m.groupValues[2].toLong()
            val frac = m.groupValues[3]
            val ms = when (frac.length) { 1 -> frac.toLong()*100; 2 -> frac.toLong()*10; 3 -> frac.toLong(); else -> 0 }
            LyricLine((min*60+sec)*1000+ms, m.groupValues[4])
    ...
    private suspend fun libraryAction(kind: String, trackId: String): Boolean = withContext(Dispatchers.IO) {
        var uid = accountUid
        if (uid == null) {
            val status = getResult("$API/account/status")
            uid = status.optJSONObject("account")?.opt("uid")?.toString()
            accountUid = uid
        }
        if (uid.isNullOrBlank()) throw IOException("Не удалось определить ID аккаунта")
        val body = FormBody.Builder().add("track-ids", trackId.substringBefore(':')).build()
        val req = authBuilder("$API/users/$uid/$kind/tracks/add-multiple").post(body).build()
        http.newCall(req).execute().use { r ->
            if (!r.isSuccessful) throw IOException("$kind HTTP ${r.code}: ${r.body?.string().orEmpty().take(120)}")
            true
        }
    }
    ...
    suspend fun searchTracks(text: String): List<Track> {
        ...
        val result = getResult("$API/search?text=${URLEncoder.encode(text, "UTF-8")}&type=track&page=0&nocorrect=false")
        val arr = result.optJSONObject("tracks")?.optJSONArray("results") ?: JSONArray()
    ...
    suspend fun audioUrl(trackId: String): String = withContext(Dispatchers.IO) {
        val infos = getResultArray("$API/tracks/$trackId/download-info")
        var chosen: JSONObject? = null
        for (i in 0 until infos.length()) {
            val candidate = infos.getJSONObject(i)
            if (candidate.optString("codec") == "mp3") {
                val current = chosen
                if (current == null || candidate.optInt("bitrateInKbps") > current.optInt("bitrateInKbps")) {
                    chosen = candidate
                }
            }
        }
        val info = chosen ?: throw IOException("Для трека не найден MP3-поток")
        val xml = executeText(authRequest(info.getString("downloadInfoUrl")))
        fun tag(name: String): String {
            val value = Regex("<$name>(.*?)</$name>").find(xml)?.groupValues?.get(1)
                ?: throw IOException("Нет <$name> в download-info")
        ...
        val ts = tag("ts")
        val s = tag("s")
        val sign = md5(SIGN_SALT + path.drop(1) + s)
    ...
    suspend fun waveFeedback(type: String, trackId: String? = null, batchId: String? = null, playedSeconds: Long? = null) = withContext(Dispatchers.IO) {
        val form = FormBody.Builder()
            .add("type", type)
            .add("timestamp", (System.currentTimeMillis() / 1000.0).toString())
            .apply {
                trackId?.let { add("trackId", it) }
                playedSeconds?.let { add("totalPlayedSeconds", it.toString()) }
                if (type == "radioStarted") add("from", "mobile-radio-user-onyourwave")
            }
            .build()
        val batchSuffix = batchId?.takeIf { it.isNotBlank() }?.let {
            "?batch-id=${URLEncoder.encode(it, "UTF-8")}"
        }.orEmpty()
        val url = "$API/rotor/station/user:onyourwave/feedback$batchSuffix"
        http.newCall(authBuilder(url).post(form).build()).execute().use { r ->
            val text = r.body?.string().orEmpty()
            if (!r.isSuccessful) throw IOException("Feedback HTTP ${r.code}: ${text.take(160)}")
        }
    }
    ...
    suspend fun playlistTracks(uid: String, kind: String): List<Track> {
        val result = getResult("$API/users/${URLEncoder.encode(uid, "UTF-8")}/playlists/${URLEncoder.encode(kind, "UTF-8")}")
    ...
    suspend fun podcasts(): List<HomeCard> {
        val result = getResult("$API/landing3/podcasts")
        val ids = result.optJSONArray("podcasts") ?: JSONArray()
    ...
    private fun executeText(req: Request): String = http.newCall(req).execute().use { r ->
        if (!r.isSuccessful) throw IOException("HTTP ${r.code}: ${r.body?.string().orEmpty().take(200)}")
